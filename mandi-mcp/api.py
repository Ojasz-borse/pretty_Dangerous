...
async def grade_crop_image(file: UploadFile = File(...)):
    """
    Detect crop type AND grade quality using:
    - Gemini 1.5 Flash Vision (primary, requires GEMINI_API_KEY env var)
    - Pre-trained ImageNet MobileNetV2 (automatic fallback, no API key needed)
    Returns: crop_name, grade (A/B/C), confidence_percentage, reason, detail_scores
    """
    ...
    try:
        ...
        result      = await vision_svc.detect_crop_with_ai(image_bytes, GEMINI_API_KEY)
        ...
        if result.get("status") == "not_crop":
            raise HTTPException(
                status_code=422,
                detail=result.get("error", "This does not appear to be a crop image.")
            )
