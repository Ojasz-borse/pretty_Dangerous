...
export default function FarmerDashboard() {
    ...
    const renderContent = () => {
        switch (activeTab) {
            case 'price': return <RealTimePrice selectedCrop={selectedCrop} setSelectedCrop={setSelectedCrop} district={district} />;
            case 'prediction': return <PricePrediction cropName={selectedCrop} district={district} />;
            case 'sell': return <SellRecommendation cropName={selectedCrop} district={district} />;
            case 'demand': return <DemandForecast district={district} state={state} />;
            case 'detect': return <CropDetection />;
            case 'trust': return <TrustScore />;
            case 'logistics': return <LogisticsCalculator cropName={selectedCrop} pickupDistrict={district} pickupState={state} />;
            default: return null;
        }
    };
        ...
        <div className="min-h-screen relative">
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-4">
                            ...
                            <div><h1 className="text-xl sm:text-2xl font-bold"><span className="text-gradient-green">Krishi</span><span className="text-white/90">Setu</span></h1><p className="text-xs text-white/40 tracking-wider uppercase">Smart Agri Intelligence</p></div>
                        </div>
                        <div className="flex items-center gap-3">
                            <div className="hidden sm:flex items-center gap-2 badge-glass px-3 py-1.5 rounded-full text-xs"><Clock className="w-3 h-3 text-emerald-400" /><span>{currentTime}</span></div>
                            <div className="hidden sm:flex items-center gap-2 badge-glass px-3 py-1.5 rounded-full text-xs"><Activity className="w-3 h-3 text-emerald-400" /><span>{district}, {state}</span></div>
                        </div>
                    </div>
                </div>
            </header>
            ...
            <main className="max-w-7xl mx-auto px-4 sm:px-6 py-6 space-y-6">
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                    {[
                        { label: "Today's Best Price", value: stats.bestPrice, sub: '/Quintal', icon: DollarSign, glow: 'glow-green', gradient: 'from-emerald-400 to-green-500', delay: '' },
                        { label: 'Price Trend', value: stats.trend, sub: 'Current market', icon: TrendingUp, glow: 'glow-blue', gradient: 'from-blue-400 to-cyan-500', delay: 'delay-100' },
                        { label: 'Demand Index', value: stats.demand, sub: 'Region status', icon: Target, glow: 'glow-purple', gradient: 'from-purple-400 to-violet-500', delay: 'delay-200' },
                        { label: 'Trust Score', value: stats.trustScore, sub: 'Reliable farmer', icon: Star, glow: 'glow-amber', gradient: 'from-amber-400 to-yellow-500', delay: 'delay-300' },
                    ].map((stat, i) => (
                        <div key={i} className={`stat-card-3d animate-fade-in-up ${stat.delay}`}>
                            <div className={`stat-card-inner ${stat.glow}`}>
                                <div className="flex items-start justify-between mb-3"><div className={`w-10 h-10 rounded-xl bg-gradient-to-br ${stat.gradient} flex items-center justify-center shadow-lg`}><stat.icon className="w-5 h-5 text-white" /></div></div>
